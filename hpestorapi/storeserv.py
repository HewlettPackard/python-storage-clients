...
class StoreServ:
    ...
    def timeout(self):
        ...
        Number of seconds that Rest API client waits for a response from HPE
        StoreServ before generating a timeout exception. Different timeouts
        for connection setup and for getting first piece of data can be used. In
        this case, use tuple(float, float) with the first value being a
        connection delay and with the second value being a read delay.
        Alternatively, you can use one float value for both types of
        timeouts. 'None' value can be used instead not to limit the device
        response time. Default value: (1, None).
        """
